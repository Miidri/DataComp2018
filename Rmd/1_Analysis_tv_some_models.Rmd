...
p1 <- xgb.importance(model = xgb_model) %>% 
  data.frame(.,Feature2 = c("放送時刻(時間)", "番組クラスタ","チャンネル","放送時間","大分類平均視聴率","放送曜日","放送形式",
                            "大分類","中分類","小分類","新番特番フラグ", "最終回コード")) %>% 
...
p2 <- data.frame(test_data,pred = pred_test) %>% 
  ...
  labs(y ="xgboostによる予測視聴率", x="実際の視聴率", 
       title="テスト期間: 2018/2/1 - 2018/4/1, 標本数: 17466, RMSLE = 0.0053") 
```
...
p1 <- data.frame(test_data,pred = pred_test) %>% 
  ...
  labs(y ="従来手法による推定視聴率", x="実際の視聴率", 
       title="テスト期間: 2018/2/1 - 2018/4/1, 標本数: 17466,  RMSE = 0.0065") 
```
