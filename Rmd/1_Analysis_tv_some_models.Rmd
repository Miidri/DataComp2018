...
alltime2 %>% 
  ...
  left_join(LogCluster1 %>% 
            ...
            by="program_code") -> tmp
tmp1 <- tmp %>% 
  left_join(tmp[1:80000,] %>% group_by(ban_code1) %>% summarise_at(c("all_watch_rate"),fn2),by = "ban_code1") %>% 
  left_join(tmp[1:80000,] %>% group_by(Hour) %>% summarise_at(c("all_watch_rate"),fn2),by = "Hour")
```

```{r}
train_data <- tmp1[1:80000,] %>% filter(all_watch_rate %>% is.na() %>% !.)
test_data <- tmp1[80001:99598,] %>% filter(all_watch_rate %>% is.na() %>% !.)
```
