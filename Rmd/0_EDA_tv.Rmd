...
realtime <- orgn2 %>% 
  ...
  summarise(house_watch_time = sum(watch_time)) %>% 
  ungroup() %>% 
...
realtime <- orgn2 %>% 
  ...
  summarise(house_watch_time = mean(watch_time)) %>% 
  ungroup() %>% 
  group_by(station_code, program_start_time) %>% 
  summarise(all_watch = sum(house_watch_time) / program_time) %>% 
  ungroup()
```
