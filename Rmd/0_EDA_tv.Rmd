...
alltime2 %>% 
  left_join(program %>% 
            ...
            by = c("station_code", "program_start_time")) %>% 
  ggplot(aes(x=all_watch_rate,y=mean_rate)) + 
  geom_point(aes(colour = factor(ban_code1))) +
  geom_smooth(method = "lm", formula = formula) +
  stat_poly_eq(formula = formula, parse = TRUE, label.y = 0.2, size=12, color = "magenta3") + 
  theme_set(theme_classic(base_size = 18,base_family = "HiraKakuPro-W3")) +
  guides(colour = guide_legend(title = "大分類")) +
  labs(y ="予測視聴率", x="実測値") 
```
...
alltime2 %>% 
  left_join(program %>% 
              select(station_code, program_start_time, sin_toku),
            by = c("station_code", "program_start_time")) %>% 
  ggplot(aes(x=all_watch_rate,y=mean_rate)) + 
  geom_point(aes(colour = factor(sin_toku))) +
  geom_smooth(method = "lm", formula = formula) +
  stat_poly_eq(formula = formula, parse = TRUE, label.y = 0.2, size=12, color = "magenta3") + 
  theme_set(theme_classic(base_size = 18,base_family = "HiraKakuPro-W3")) +
  guides(colour = guide_legend(title = "番組特性")) +
  labs(y ="予測視聴率", x="実測値") 
```
...
alltime2 %>% 
  left_join(program %>% 
              select(station_code, program_start_time, final_code),
            by = c("station_code", "program_start_time")) %>% 
  ggplot(aes(x=all_watch_rate,y=mean_rate)) + 
  geom_point(aes(colour = factor(final_code))) +
  geom_smooth(method = "lm", formula = formula) +
  stat_poly_eq(formula = formula, parse = TRUE, label.y = 0.2, size=12, color = "magenta3") + 
  theme_set(theme_classic(base_size = 18,base_family = "HiraKakuPro-W3")) +
  guides(colour = guide_legend(title = "最終回フラグ")) +
  labs(y ="予測視聴率", x="実測値") 
```
# 予測モデルの構築
