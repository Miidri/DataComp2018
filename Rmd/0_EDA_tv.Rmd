...
```{r}
source('~/Desktop/DataComp2018/script/mean_watch_rate.R')
```
...
```{r}
ggplot(alltime,aes(factor(station_code), all_watch_rate)) +
  geom_violin() + 
  theme_set(theme_classic(base_size = 18,base_family = "HiraKakuPro-W3")) +
  labs(x = "視聴率") 
```
...
alltime %>% 
  ...
  ggplot(aes(x=program_start_time, real_watch_rate,color = station_code)) +
    geom_line() +
...
```{r}
# 必要な情報を抽出
tmp1 <- alltime %>% select(station_code, program_start_time, all_watch_rate) %>% filter(station_code == 1)
tmp2 <- alltime %>% select(station_code, program_start_time, all_watch_rate) %>% filter(station_code == 2)
tmp3 <- alltime %>% select(station_code, program_start_time, all_watch_rate) %>% filter(station_code == 3)
tmp4 <- alltime %>% select(station_code, program_start_time, all_watch_rate) %>% filter(station_code == 4)
tmp5 <- alltime %>% select(station_code, program_start_time, all_watch_rate) %>% filter(station_code == 5)
tmp6 <- alltime %>% select(station_code, program_start_time, all_watch_rate) %>% filter(station_code == 6)
tmp7 <- alltime %>% select(station_code, program_start_time, all_watch_rate) %>% filter(station_code == 7)
```

```{r}
MeanWatchRate(tmp = tmp1) -> ans
```
...
for(i in 1:100){
  for(j in t:10000){
    if(tmp1_$pre1[i] >= tmp1$program_start_time[j] && tmp1_$pre1[i] < tmp1$program_start_time[j+1]){
      tmp1_$rate1[i] = tmp1$all_watch_rate[j]
      t = j
      break
    }
  }
...
k <- foreach (i = 1:10, .combine = c) %do% {
    for(j in 1:10000){
      if(tmp1_$pre1[i] >= tmp1$program_start_time[j] && tmp1_$pre1[i] < tmp1$program_start_time[j+1]){
        return(tmp1$all_watch_rate[j])
        break
       }
    }
}
