...
- 総カラム数: 47498546件
- Distinct(time,id): 46885392件
- Distinct(time,id,os): 46886842件	
- 閲覧時間1秒以上: 46791232件
- 閲覧時間1秒以上かつDistinct(time,id): 46789430件
- 閲覧時間1秒以上かつDistinct(time,id,os): 46790286件

```{r}
# 同一時刻, 同一id
dbGetQuery(
  con,"SELECT COUNT(*) FROM (SELECT DISTINCT web_start_datetime, house_num FROM processed.t_weblog) AS test;")
# 閲覧時間1秒以上
dbGetQuery(
  con,"SELECT COUNT(*) FROM processed.t_weblog WHERE web_time <> 0;")
# 閲覧時刻i秒以上かつ同一時刻, 同一id
dbGetQuery(
  con,"SELECT COUNT(*) 
        FROM (
...
(
  SELECT DISTINCT
      t1.house_num, t1.web_start_datetime, t1.os
  FROM
  (
    SELECT
    house_num,
