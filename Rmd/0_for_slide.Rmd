...
```{r}
job1 <- job %>% arrange(job_code) %>% dplyr::collect()
tmp <- sample %>% 
  dplyr::collect() %>% 
  left_join(
    data.frame(job1,job2 = 
                 c("給料事務/技術職","給料技能/作業職","経営/管理職","農業/漁業/林業",
                   "専門職/自由業","商工自営業","乳幼児/未就学児","小学生","中学生",
                   "高校生","大学生","各種学校生/予備校生","専業主婦",
                   "パートタイマー","無職","その他")),
    by="job_code") %>% 
  group_by(job2) %>% 
  summarise(num = n()) %>% 
...
p4 <- ggplot(tmp,
       aes(factor(job2, 
                  levels = 
                    c("大学生","高校生","各種学校生/予備校生","中学生","小学生",
                      "乳幼児/未就学児","専業主婦","パートタイマー","経営/管理職",
                      "給料技能/作業職","給料事務/技術職","専門職/自由業","農業/漁業/林業",
                      "商工自営業","無職","その他")),
           num)) +
    theme_set(theme_classic(base_size = 18,base_family = "HiraKakuPro-W3")) +
    geom_bar(stat = "identity",fill = "steelblue") +
...
tmp <- alltime %>% 
  ...
  inner_join(performer %>% 
              select(station_code,program_start_time,regular, guest) %>% 
              dplyr::collect() %>% 
              mutate(program_start_time = 
                       as_datetime(program_start_time, tz="Asia/Tokyo")),
            by = c("program_start_time","station_code"))
# レギュラー, ゲスト空白を削除
regular <- tmp %>% 
  mutate(regular = str_replace_all(regular, fixed(" "), "")) %>% 
  select(-guest) %>% 
  filter(regular != "") %>% 
  mutate(regular = strsplit(regular,",")) %>% 
  unnest(regular) %>% 
  mutate(regular = str_trim(regular, side="both"))

guest <- tmp %>% 
  mutate(guest = str_replace_all(guest, fixed(" "), "")) %>% 
  select(-regular) %>% 
  filter(guest != "") %>% 
...
p1 <- regular %>% 
  ...
  ungroup() %>% 
  ggplot(aes(x=num,y=rate)) +
  geom_point() +
  theme_set(theme_classic(base_size = 18,base_family = "HiraKakuPro-W3")) +
  labs(x="レギュラー番組出演回数",y="出演者の平均獲得視聴率")
...
toplist <- regular %>% 
  ...
  summarise(num = n(),
         rate = mean(all_watch_rate,na.rm = TRUE)) %>%
  ungroup() %>% 
  filter(num > 50) %>% #出演50回以上
  top_n(20,rate)
p2 <- toplist %>% 
  inner_join(regular,by="regular") %>% 
  ggplot(aes(x=factor(regular),y=all_watch_rate))+  
  geom_boxplot() + 
  # stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1)) + 
  theme_set(theme_classic(base_size = 18,base_family = "HiraKakuPro-W3")) +
  theme(axis.text.x = element_text(angle = 30,hjust = 1)) +
  labs(x="レギュラー番組出演者",y="出演者の獲得視聴率")
```

```{r}
p <- grid.arrange(p1,p2,nrow=2)
ggsave(file = "~/Desktop/DataComp2018/image/basic/出演者情報.png", plot = p, dpi = 100, width = 10, height = 9.3)
```
