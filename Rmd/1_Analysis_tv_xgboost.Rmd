...
alltime2 %>% 
  ...
  makedummies(basal_level = TRUE) -> tmp
```
...
```{r}
model <- lm(all_watch_rate ~ factor(ban_code1) + factor(ban_code2) + factor(sin_toku) + factor(br_format) 
            + factor(final_code) + program_time + factor(Wday) + factor(Hour), 
            data = train_data)
pred <- predict(model,test_data)
plot(test_data$all_watch_rate,pred)
```
